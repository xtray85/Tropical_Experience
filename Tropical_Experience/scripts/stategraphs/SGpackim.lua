...
local PACKIM_FIRE_DELAY_MIN = 5
local PACKIM_FIRE_DELAY_MAX = 10
...
local events=
{
	...
	EventHandler("doattack", function(inst, data)
		inst.contador = inst.contador + 1
		if inst.contador > 50 then
		inst.contador = 0
	...
	State{
		...
		events=
		{
			EventHandler("animover",
				function(inst)
					local packimstate = inst.ChesterState
					if inst.tryeat(inst) then --Do I contain anything I want to eat?
						if packimstate == inst.ChesterState then
...
--				inst.fire_interval = math.random(PACKIM_FIRE_DELAY_MIN, PACKIM_FIRE_DELAY_MAX)
