...
local function onattack(inst, data)
    ...
    inst.sg:HasStateTag("busy")) then
		...
		local cobra = GetClosestInstWithTag("snake", inst, 17)
...
local states=
{
	...
	State{
        ...
        timeline=
        {
	            TimeEvent(37*FRAMES, function(inst) inst.components.combat:DoAttack(inst.sg.statemem.target) end),
            		TimeEvent(38*FRAMES, function(inst) inst.sg:RemoveStateTag("attack") end),

			TimeEvent(0*FRAMES, function(inst) inst.SoundEmitter:PlaySound("dontstarve/creatures/leif/foley") end),
			TimeEvent(15*FRAMES, function(inst) inst.SoundEmitter:PlaySound("dontstarve/creatures/leif/attack_VO") end),
			TimeEvent(22*FRAMES, function(inst) inst.SoundEmitter:PlaySound("dontstarve/creatures/leif/swipe") end),
			TimeEvent(32*FRAMES, function(inst) inst.SoundEmitter:PlaySound("dontstarve/creatures/leif/foley") end),
        },
...
return StateGraph("jungletreeguard", states, events, "idle", actionhandlers)
