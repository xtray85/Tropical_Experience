            ...
            (TheNet:GetPVPEnabled() or not (inst.WINDSTAFF_CASTER_ISPLAYER and v:HasTag("player"))) then
                ...
                if inst.WINDSTAFF_CASTER_ISPLAYER then
                    if v:HasTag("player") then
                        damage = 30 * TUNING.PVP_DAMAGE_MOD
                    end
                    if v:HasTag("twister") then
                        damage = 0
                    end
                    v.components.combat:GetAttacked(inst, damage, nil, "staff_twisterwind")
                else
                    ...
                    v.components.combat:GetAttacked(inst, damage, nil, "twister_wind")
                end
...
return StateGraph("twistertornado", states, events, "empty")
