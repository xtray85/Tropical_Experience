...
local function make_master_fn()
	...
	inst:DoTaskInTime(0.02, function() 
		inst:ListenForEvent( "clocktick", function(world, data)
			...
			local time_left_till_aporkalypse = aporkalypse and math.max(aporkalypse:GetBeginDate() - (TheWorld.state.cycles + TheWorld.state.time) * TUNING.TOTAL_DAY_TIME, 0) or 0
--			print("time_left:",time_left_till_aporkalypse	)
			...
			if inst.rewind then
				...
				if aporkalypse then
					...
					time_left_till_aporkalypse = aporkalypse and math.max(aporkalypse:GetBeginDate() - (TheWorld.state.cycles + TheWorld.state.time) * TUNING.TOTAL_DAY_TIME, 0) or 0
					...
					aporkalypse:ScheduleAporkalypse((TheWorld.state.cycles + TheWorld.state.time) * TUNING.TOTAL_DAY_TIME + time_left_till_aporkalypse)
				end
