...
local function dig_up(inst)
    if inst.components.spawner:IsOccupied() then
        inst.components.lootdropper:SpawnLootPrefab("snake_poison")
    end
...
local function onoccupied(inst)
    ...
    elseif not TheWorld.state.iscaveday and not TheWorld.state.isspring then
        startspawning(inst)
    end
end

local function OnIsNotDay(inst, iscavenight)
    if not TheWorld.state.iscaveday and not TheWorld.state.isspring and
...
local function SetSpringMode(inst)
    ...
    if (not inst.springmode and TheWorld.state.isspring) or inst.springmode == nil then
        ...
        inst:StopWatchingWorldState("stopcaveday", OnIsNotDay)
        stopspawning(inst)
...
local function SetNormalMode(inst)
    ...
    if (inst.springmode and not TheWorld.state.isspring) or inst.springmode == nil then
        ...
        inst:WatchWorldState("stopcaveday", OnIsNotDay)
        OnIsNotDay(inst, not TheWorld.state.iscaveday)
...
return Prefab("snake_hole", fn, assets, prefabs)
