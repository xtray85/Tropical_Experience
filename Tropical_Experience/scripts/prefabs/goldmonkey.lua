...
local brain = require "brains/goldmonkbrain"
...
local function OnAttacked2(inst, data)
    ...
    local x, y, z = inst.Transform:GetWorldPosition()
    local ents = TheSim:FindEntities(x, y, z, 30, { "tiki" })
    for i, v in ipairs(ents) do
        if v ~= inst then
            v.components.combat:SuggestTarget(data.attacker)
            SetHarassPlayer(v, nil)
...
local function FindTargetOfInterest(inst)
    ...
    if inst.harassplayer == nil and inst.components.combat.target == nil then
        ...
        for i = 1, #targets do
            ...
            if target.components.inventory ~= nil and math.random() < (target.components.inventory:FindItem(IsBanana) ~= nil and .6 or .15) and not target:HasTag("tiki") then
...
local function shouldKeepTarget(inst, target)
    ...
    return inst.components.combat:CanTarget(target) and not target:HasTag("tiki")
end
...
local function SetNormalMonkey(inst)
    ...
    inst:SetBrain(brain)
    inst.AnimState:SetBuild("goldmonkey_build")
