...
local guardbuilds = { "wanawana_build" }

local function NormalRetargetFn(inst)
	local exclude_tags = { "playerghost", "INLIMBO", "tiki" }
...
local function GuardRetargetFn(inst)
    ...
    if not defenseTarget.happy then
        local invader = FindEntity(defenseTarget, SpringCombatMod(TUNING.PIG_GUARD_TARGET_DIST), nil, { "character" }, { "guard", "INLIMBO" , "tiki"})
        if invader ~= nil and
...
local function GuardKeepTargetFn(inst, target)
    if not inst.components.combat:CanTarget(target) or
        ...
        (target:HasTag("guard") and target:HasTag("pig")) or target:HasTag("tiki") then
        return false
    end
...
return Prefab("wanawanatiki", guard, assets, prefabs)
