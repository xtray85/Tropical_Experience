...
local function SpawnTreeManGrove(spawn_point)
    local plant = SpawnPrefab("tree_mangrove")
...
local function SpawnTreeManGrovePre(inst)
		local pt = inst:GetPosition()
		local spawn_point = GetSpawnPoint(pt)
		if spawn_point ~= nil then
			local plant = SpawnTreeManGrove(spawn_point)
			inst:Remove()
		else
		inst.tentativas = 	inst.tentativas - 1
		if inst.tentativas and inst.tentativas > 1 then
		inst:DoTaskInTime(1,SpawnTreeManGrovePre)
		end
...
return Prefab("mangrovespawner", fn, assets, prefabs)
