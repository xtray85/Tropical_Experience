...
local function SpawnFishInHole(spawn_point)
    local plant = SpawnPrefab("fishinhole")
...
local function SpawnFishInHolePre(inst)
		local pt = inst:GetPosition()
		local spawn_point = GetSpawnPoint(pt)
		if spawn_point ~= nil then
			local plant = SpawnFishInHole(spawn_point)
			inst:Remove()
		else
		inst.tentativas = 	inst.tentativas - 1
		if inst.tentativas and inst.tentativas > 0 then
		inst:DoTaskInTime(1,SpawnFishInHolePre)
		end
...
return Prefab("fishinholewaterspawner", fn, assets, prefabs)
