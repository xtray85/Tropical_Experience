WalkAway = Class(BehaviourNode, function(self, inst, hunterparams, see_dist, safe_dist, fn, runhome)
    BehaviourNode._ctor(self, "WalkAway")
...
function WalkAway:__tostring()
    return string.format("WALKAWAY %f from: %s", self.safe_dist, tostring(self.hunter))
end

function WalkAway:GetRunAngle(pt, hp)
    ...
    --print(string.format("WalkAway:GetRunAngle me: %s, hunter: %s, run: %2.2f", tostring(pt), tostring(hp), angle))
