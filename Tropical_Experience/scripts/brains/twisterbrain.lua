    ...
    local root =
        PriorityNode(
        {
            ...
            WhileNode(function() return self.inst.sg:HasStateTag("running") or
                (self.inst.CanCharge and self.inst.components.combat.target and self.inst.components.combat.target:GetPosition():Dist(self.inst:GetPosition()) >= TWISTER_ATTACK_RANGE) end, 
            ...
            Wander(self.inst, function() return self.inst:GetPosition() end, 20, wandertimes),
        }, .25)
