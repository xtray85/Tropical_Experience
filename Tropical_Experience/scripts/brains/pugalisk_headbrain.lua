...
local function doGazeTest(inst)
    ...
    if inst.components.combat.target and not self.inst:HasTag("tail") and inst:GetDistanceSqToInst(inst.components.combat.target) > 5*5 and not inst.sg:HasStateTag("busy") then
