...
local function nibble_trap(inst)
local armadilha = GetClosestInstWithTag("lobstertrap", inst, 4)
if armadilha and armadilha.replica.inventoryitem and armadilha.replica.inventoryitem:IsHeld() then return end
if armadilha then
if TheWorld.Map:IsOceanAtPoint(armadilha.Transform:GetWorldPosition()) then
return BufferedAction(inst, armadilha, ACTIONS.WALKTO)
end
end
end
...
local armadilha = GetClosestInstWithTag("lobstertrap", inst, 8)
if armadilha and armadilha.replica.inventoryitem and not armadilha.replica.inventoryitem:IsHeld() then
if armadilha and armadilha.components.trap and armadilha.components.trap.bait ~= nil and TheWorld.Map:IsOceanAtPoint(armadilha.Transform:GetWorldPosition()) then
return BufferedAction(inst, armadilha, ACTIONS.WALKTO)
