...
local GoldMonkeyBrain = Class(Brain, function(self, inst)
...
local function EatFoodAction(inst)
    ...
    if inst.components.inventory ~= nil and inst.components.inventory:GetEquippedItem(EQUIPSLOTS.HEAD) == nil then
        for i, item in ipairs(ents) do
            if item.components.equippable ~= nil and
                ...
                item:IsOnValidGround() and item.prefab ~= "goldnugget" then
    ...
    for i, item in ipairs(ents) do
        if item.components.pickable ~= nil and
            ...
            (item.prefab == "worm" or ItemIsInList(item.components.pickable.product, ValidFoodsToPick)) and item.prefab ~= "goldnugget" then
